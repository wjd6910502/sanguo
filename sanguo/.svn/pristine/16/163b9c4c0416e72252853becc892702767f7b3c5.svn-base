function OnCommand_TongQueTaiSetHeroInfo(player, role, arg, others)
	...
	if table.getn(arg.hero) < 3 then
		...
		player:Log("OnCommand_TongQueTaiSetHeroInfo, error=TONGQUETAI_SET_HERO_COUNT_LESS")
		return
	end
	...
	for i = 1, table.getn(arg.hero) do
		...
		if h == nil then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_TongQueTaiSetHeroInfo, error=TONGQUETAI_SET_HERO_NO_HERO")
			return
		end
	end
