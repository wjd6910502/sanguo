...
class GMCmd_GetRoleForbidLogin_Re : public GNET::Protocol
{
	...
	{
		GLog::log(LOG_INFO,
		          "gmadapterd::GMCmd_GetRoleForbidLogin_Re::Process, retcode=%d, begintime=%.*s, lasttime=%d, desc=%.*s",
		...
		if(retcode == 0)
		{
			snprintf(buf, sizeof(buf),
			         "<cmd_command cmd_data=\"GetRoleForbidLogin\" return=\"true\" begintime=\"%.*s\" lasttime=\"%d\" desc=\"%.*s\"/>",
					 (int)begintime.size(), (char*)begintime.begin(), lasttime, (int)desc.size(), (char*)desc.begin());
		}
		else
		{
			snprintf(buf, sizeof(buf),
			         "<cmd_command cmd_data=\"GetRoleForbidLogin\" return=\"false\" desc=\"%.*s\"/>",
