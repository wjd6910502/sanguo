...
class AudienceGetOperationRe : public GNET::Protocol
{
	...
	{
		GLog::log(LOG_INFO, "GAMED::AudienceGetOperationRe, retcode=%d, roleid=%ld, room_id=%d, fight_robot=%d, robot_seed=%d, robot_type=%d", 
				retcode, roleid, room_id, fight_robot, robot_seed, robot_type);
		...
		char win_flag[1024];
		snprintf(win_flag, sizeof(win_flag), "52:%d:%d:%d:%d:%d:", retcode, room_id, fight_robot, robot_seed, robot_type);
		msg = win_flag;
