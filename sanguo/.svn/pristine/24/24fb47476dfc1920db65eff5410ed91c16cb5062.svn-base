...
public:
	...
	//连接服务器超时，此时网络模块仍在后台工作(如要关闭请调用Close)
	//virtual void OnOpenTimeout() = 0;
	//客户端版本太老，需要退出更新, 此时网络模块已关闭(需要手动调用Open重连)
	virtual void OnClientOutOfDate() = 0;
	...
	//发送命令给服务器时超时，弹板提示玩家网络正在连接中并阻止玩家可能可能的操作，此时网络模块仍在后台工作(如要关闭请调用Close)
	virtual void OnSendGameProtocolTimeout() = 0;
	//网络状态已恢复，如果有timeout弹板，则关闭弹板，否则不用处理
	virtual void OnTimeoutClear() = 0;
...
public:
	FastUDPSession(bool p2p, Connection *conn): _p2p(p2p), _opened(false), _index_stub(0), _received_index(0), _need_send_ack(false),
	                                            _prev_real_send_ms(0), _conn(conn)
	{
	}
	...
	void SendAck();
	void TriggerSend(bool force, int retrans_time_ms);
};
...
public:
	P2PHelper(Connection *conn): _net_type(P2P_NET_TYPE_UNKNOWN), _public_port(0), _need_report(false), _get_net_type_status(P2P_GET_NET_TYPE_STATUS_NONE),
	             _get_net_type_status_change_time(0), _get_net_type_magic(0), _prev_update_time(0), _make_hole_status(P2P_MAKE_HOLE_STATUS_NONE),
	             _make_hole_magic(0), _make_hole_peer_port(0), _make_hole_time(0), _conn(conn)
	{
	}
...
public:
	Latency(int max_interval_ms, Connection *conn): _sample_interval_max(max_interval_ms*1000), _conn(conn) { Reset(); }
...
class Connection: public IntervalTimer::Observer
{
	...
	enum CONN_STATUS_TIMEOUT
	{
		...
		CONN_STATUS_TIMEOUT_WAIT_TRANS_AUTH_RESULT	=	10,
	...
	std::map<int,int> _game_protocol_timeout; //prot_id=>timeout
	bool _game_protocol_timeouting;
	...
	unsigned short _prev_server_send_time_4_udp;
	int64_t _prev_server_send_time_4_udp_local_time;
	...
	std::vector<std::string> _server_logs;
	std::map<std::string, int64_t> _kv_map;

	GateClient *_gate_client;
	TransClient *_trans_client;
	UDPTransClient *_udp_trans_client;
	StatusClient *_status_client;
...
public:
	Connection(): _status(CONN_STATUS_NONE), _can_send_game_protocol(false), _timeout(0), _callback(0), _gate_port(0), _status_port(0),
	              ...
	              _first_game_protocol_id(1), _game_protocol_timeouting(false), _client_received_game_protocol_count(0),
	              _server_received_game_protocol_count(0), _prev_update_time(0),
	              _udp_session(false,this), _p2p_helper(this), _p2p_udp_session(true,this),
	              _udp_is_init(false), _server_time_offset(0), _prev_server_send_time_4_tcp(0),
	              _prev_server_send_time_4_tcp_local_time(0), _prev_server_send_time_4_udp(0), _prev_server_send_time_4_udp_local_time(0),
	              _prev_server_send_time_4_pvp(0), _prev_server_send_time_4_pvp_local_time(0), _latency(1100,this), _pvp_latency(30,this),
	              _gate_client(0), _trans_client(0), _udp_trans_client(0), _status_client(0)
	{
	}
	~Connection() { ReleaseTimer(); }
...
public:
	...
	void WriteServerLog(const std::string& log);

	void KV_Clear();
	void KV_Set(const std::string& key, int val);
	void KV_Change(const std::string& key, int chg);
...
public:
	...
	void UpdateServerReceivedCount(int server_received_count);
...
public:
	...
	unsigned short GetPrevServerSendTime4TCP() const { return _prev_server_send_time_4_tcp; }
	int64_t GetPrevServerSendTime4TCPLocalTime() const { return _prev_server_send_time_4_tcp_local_time; }
	...
	UDPTransClient* GetUDPTransClient() const { return _udp_trans_client; }

private:
	void ChangeStatus(CONN_STATUS status);
	void AddHistory(const GameProtocol& data, int timeout);
};
