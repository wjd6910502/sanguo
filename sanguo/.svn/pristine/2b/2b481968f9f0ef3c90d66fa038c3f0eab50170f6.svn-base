function OnCommand_EquipmentLevelUp(player, role, arg, others)
	...
	if typ_flag == 0 then
		...
		player:Log("OnCommand_EquipmentLevelUp, error=EQUIPMENT_TYP_ERR")
		return
	end
	...
	if find_equipment == nil then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentLevelUp, error=EQUIPMENT_NOT_EXIST")
		return
	end
	...
	if find_equipment._equipment_pro._level_up >= role._roledata._status._level*2 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentLevelUp, error=EQUIPMENT_MAX_LEVEL")
		return
	end
	...
	if equipment_lv_info == nil then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentLevelUp, error=EQUIPMENT_LEVEL_SYS_ERR")
		return
	end
	...
	if need_money > role._roledata._status._money then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentLevelUp, error=EQUIPMENT_LEVEL_MONEY_LESS")
		return
	end
	...
	local old_level = find_equipment._equipment_pro._level_up
	...
	--成就修改
	TASK_ChangeCondition_Special(role, G_ACH_TYPE["EQUIPMENT_LEVELUP"], find_equipment._equipment_pro._level_up, old_level, 1)
