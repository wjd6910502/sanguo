...
class UDPS2CGameProtocols : public GNET::Protocol
{
	...
	void Process(Manager *manager, Manager::Session::ID sid)
	{
		...
		if(signature != UDPSignature(tmp, conn->GetKey1()))
		{
		...
		conn->UpdatePVPLatency(client_send_time, server_send_time);
		conn->FastSess_OnAck(index_ack, false);
		...
		for(auto it=protocols.begin(); it!=protocols.end(); ++it)
		{
			...
			if(conn->FastSess_IsReceived(idx, false)) continue;
			conn->FastSess_SetReceived(idx, false);
		...
		if(!protocols.empty()) conn->FastSess_SendAck(false);
	}
};
