function OnCommand_MaShuSelectFriendToHelp(player, role, arg, others)
	...
	if friend_info == nil then
		...
		player:Log("OnCommand_MaShuSelectFriendToHelp, error=MASHU_NOT_FRIEND")
		return
	end
	...
	--	resp.retcode = G_ERRCODE["MASHU_FRIEND_COUNT_MAX"]
	--	player:SendToClient(SerializeCommand(resp))
	--	return
	--end
	...
	if friend_count ~= nil then
		...
		if role._roledata._status._yuanbao < need_money then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_MaShuSelectFriendToHelp, error=MASHU_FRIEND_MONEY_LESS")
			return
		end
