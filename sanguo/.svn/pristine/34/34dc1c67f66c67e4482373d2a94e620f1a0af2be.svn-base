...
--	API_Log("OnMessage_RoleUpdateDefencePlayerPveArenaInfo, "..DumpTable(arg).." "..DumpTable(others))
	...
	if score_change > 0 then
		TASK_ChangeCondition(role, G_ACH_TYPE["YONGJIU"], G_ACH_TWENTY_TYPE["JJCSCORE"], score_change)
	end
	...
	tmp_info._operation._str = arg.operation
	tmp_info._operation._checksum = G_CHECKSUM_S["PveArenaOperation"]
	tmp_info._time = API_GetTime()
	tmp_info._score_change = score_change
	...
	tmp_info._oppo_info._photo = arg.photo
	tmp_info._oppo_info._photo_frame = arg.photo_frame
	tmp_info._oppo_score = arg.score
	tmp_info._oppo_zhanli = arg.zhanli
	for i = 1, table.getn(arg.badge_info) do
		local badge_info = CACHE.BadgeInfo()
		badge_info._pos = arg.badge_info[i].typ
		badge_info._id = arg.badge_info[i].id
		tmp_info._oppo_info._badge_map:Insert(badge_info._pos, badge_info)
	end
	...
	for i = 1, table.getn(ds_hero_info.heroinfo) do
		...
		insert_hero._skin = ds_hero_info.heroinfo[i].skin
