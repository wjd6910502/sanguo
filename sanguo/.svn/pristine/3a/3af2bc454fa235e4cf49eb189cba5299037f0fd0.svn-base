...
function PVEARENA_InitArenaRobotData()
	...
	for robot in DataPool_Array(arena_robot) do
		...
		TOP_InsertData(API_GetLuaTopList()._data._top_data, 4, CACHE.Int64(robot.robot_id), robot.robot_point, 0, robot.robot_name, photo, 0, CACHE.BadgeInfoMap(), robot.robot_level, 0, 0, 0)
	end
end
...
function PVEARENA_RefreshTime(role, now)
	...
	if times > 0 then
		...
		LIMIT_ReduceUseLimit(role, quanju.arena_free_times, tim + 1)
