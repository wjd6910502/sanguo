function OnCommand_PveArenaEndBattle(player, role, arg, others)
	...
	if arg.win_flag == 1 then
		...
		prize_id = quanju.arena_win_get_coins3
		if arg.win_type == 2 then
			prize_id = quanju.arena_win_get_coins2
		elseif arg.win_type == 3 then
	...
	if resp.score_change >0 then
		...
		for data in DataPool_Array(arenaexponent) do
			if role._roledata._pve_arena_info._score >= data.min_score and role._roledata._pve_arena_info._score <= data.max_score then
				TASK_ChangeCondition(role, G_ACH_TYPE["YONGJIU"], G_ACH_TWENTY_TYPE["JIEWEI"], data.arena_exponent_id)
				break
			end
		end		
	end
	...
	tmp_info._operation._str = table.concat(os)
	...
	msg.operation = tmp_info._operation._str
