function OnCommand_WriteHeroComments(player, role, arg, others)
	...
	if heroinfo.comment_limit == 0 then
		...
		player:Log("OnCommand_WriteHeroComments, error=HERO_COMMENTS_CAN_NOT_WRITE")
		return
	else
		if LIMIT_TestUseLimit(role, heroinfo.comment_limit, 1) == false then
			resp.retcode = G_ERRCODE["HERO_COMMENTS_MAX"]
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_WriteHeroComments, error=HERO_COMMENTS_MAX")
			return
		end
	end
	...
	if com == nil then
		--说明这个玩家是第一个评价这个武将的
		local com_list = CACHE.HeroCommentsList()
		local role_com = CACHE.HeroComments()
		...
		role_com._agree = CACHE.Int64intMap()
