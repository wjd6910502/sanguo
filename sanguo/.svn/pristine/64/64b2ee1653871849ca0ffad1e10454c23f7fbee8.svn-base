function OnCommand_Hero_Up_Grade(player, role, arg, others)
	...
	resp.retcode = Hero_up_Grade(player, role, arg.hero_id)
...
function Hero_up_Grade(player, role, hero_id)
	--数据统计日志
	...
	for grade in DataPool_Array(herograde.grade) do
		if grade.grade == hero_order then
			...
			local learn_skill = grade.unlockspeciality
			break
		end
	end
	...
	--在这里开始扣除材料，修改武将的界别
	if bexist1 then
		BACKPACK_DelItem(role, item_id1, item_count1, source_id)
	end
	if bexist2 then
		BACKPACK_DelItem(role, item_id2, item_count2, source_id)
	end
	
	--扣钱
	if cost_money > 0  then
		ROLE_SubMoney(role, cost_money, source_id)			
	end
	...
	--成就修改
	TASK_ChangeCondition_Special(role, G_ACH_TYPE["HERO_TUPO"], hero._order, hero._order-1, 1)

	HERO_UpdateHeroSkill(role, hero_id)
	--给客户端发送武将信息的修改
