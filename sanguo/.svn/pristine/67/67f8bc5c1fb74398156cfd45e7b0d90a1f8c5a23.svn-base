function OnCommand_BuyVp(player, role, arg, others)
	...
	if LIMIT_TestUseLimit(role, quanju.vp_buy_max_times, 1) == true then
		...
		if need_yuanbao == 0 then
			...
			player:Log("OnCommand_BuyVp, error=VP_SYSTEM_ERROR")
			return
		end
		if need_yuanbao > curr_yuanbao then
			resp.retcode = G_ERRCODE["VP_LESS_YUANBAO"]
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_BuyVp, error=VP_LESS_YUANBAO")
			return
		end
		--下面开始扣除元宝
		...
		TASK_ChangeCondition(role, G_ACH_TYPE["STAGE_COUNT"], G_ACH_EIGHT_TYPE["BUYVP"] , 1)
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_BuyVp, error=VP_NO_COUNT")
		return
	end
end
