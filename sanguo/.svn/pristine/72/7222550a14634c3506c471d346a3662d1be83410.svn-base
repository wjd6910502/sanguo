...
public:
	...
	inline void IncPollCount() { atomic_inc(&_cur._poll_count); }
	...
	void Dump()
	{
		...
		GLog::log(LOG_INFO, "STAT, _cmd=%d(+%d), _udp_cmd=%d(+%d), _udp_c2s=%d(+%d), _udp_s2c=%d(+%d), _msg_put=%d(+%d), _msg_run=%d(+%d), _poll_count=%d(+%d)\n",
			   ...
			   atomic_read(&tmp._poll_count), atomic_read(&tmp._poll_count)-atomic_read(&_prev_dump._poll_count)
		          );
