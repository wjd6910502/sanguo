...
class SendPvpVideoID : public GNET::Protocol
{
	...
	{
		GLog::log(LOG_INFO, "GAMED::SendPvpVideoID, roleid=%ld, video_id=%ld, win_flag=%d, score=%d, duration=%d, robot_flag=%d", roleid, video_id, win_flag, score, duration, robot_flag);
		...
		snprintf(msg, sizeof(msg), "10021:%.*s:%.*s:%.*s:%d:%d:%d:%d:%d:", (int)tmp_out.size(), (char*)tmp_out.begin(),
				(int)first_pvpinfo_out.size(), (char*)first_pvpinfo_out.begin(), 
				(int)second_pvpinfo_out.size(), (char*)second_pvpinfo_out.begin(), win_flag, match_pvp, score, duration, robot_flag); //PvpVideoID
