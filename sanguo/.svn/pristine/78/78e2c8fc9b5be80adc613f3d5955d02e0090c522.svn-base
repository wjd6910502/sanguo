...
template<typename T1, typename T2>
Marshal::OctetsStream& Marshal::OctetsStream::operator << (const std::set<T1, T2> &x) 
{
    return *this <<( MarshalContainer(x));
}
...
template<typename T1, typename T2>
const Marshal::OctetsStream& Marshal::OctetsStream::operator >> (const std::set<T1, T2> &x) const
{
	return *this >>( MarshalContainer(x));			
}
