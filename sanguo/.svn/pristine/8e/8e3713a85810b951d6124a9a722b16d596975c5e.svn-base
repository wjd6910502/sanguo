function OnCommand_MafiaOperateApplyList(player, role, arg, others)
	...
	if mafia_data == nil then
		...
		player:Log("OnCommand_MafiaOperateApplyList, error=NO_MAFIA")
		return
	end
	...
	if role._roledata._mafia._position ~= 1 and role._roledata._mafia._position ~= 2 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MafiaOperateApplyList, error=MAFIA_SET_LEVEL_ERR")
		return
	end
	...
	while apply_info ~= nil do
		if apply_info._id:ToStr() == arg.role_id then
			have_flag = true
			break
		end
	...
	if have_flag == false then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MafiaOperateApplyList, error=MAFIA_ACCEPT_NO_MEMBER")
		return
	end
	...
	if dest_role == nil then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MafiaOperateApplyList, error=MAFIA_ACCEPT_NO_ROLE")
		return
	end
	...
	if arg.accept == 1 then
		...
		if dest_role._roledata._mafia._id:ToStr() ~= "0" then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_MafiaOperateApplyList, error=MAFIA_ACCEPT_HAVE_MAFIA")
			return
		end
		...
		if mafia_info._member_map:Size() >= league_level_info.member_num then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_MafiaOperateApplyList, error=MAFIA_ACCEPT_MEMBER_MAX")
			return
		end
		...
		TASK_ChangeCondition(dest_role, G_ACH_TYPE["YONGJIU"], G_ACH_TWENTY_TYPE["MAFIAJOIN"], 1)
