...
class GMCmd_GetAccCharList : public GNET::Protocol
{
	...
	{
		GLog::log(LOG_INFO, "gamed::GMCmd_GetAccCharList::Process, account=%.*s, session_id=%d", 
				(int)account.size(), (char*)account.begin(), session_id);
		...
		if(!player)
		{
			GLog::log(LOG_INFO, "gamed::GMCmd_GetAccCharList::Process, Failure, account=%.*s, session_id=%d, not found", 
					(int)account.size(), (char*)account.begin(), session_id);
