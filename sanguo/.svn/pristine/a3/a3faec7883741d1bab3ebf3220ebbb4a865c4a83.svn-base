...
bool PWRD::Session::TryProcessXml(std::string cmd)
{
	...
	else if(cmd_data=="MailToPlayer")
	{
		GMCmd_MailToPlayer prot;
		...
		if(!cmd_command->Attribute("mailid", &prot.mailid)) return false;

		prot.session_id = _id;
		GMAdapterClient::GetInstance()->SendProtocol(prot);
		_resp_4_timeout = "<cmd_command cmd_data=\"MailToPlayer\" return=\"false\" desc=\"timeout\"/>";
