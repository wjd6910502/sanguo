function OnCommand_MafiaKickout(player, role, arg, others)
	...
	if arg.role_id == role._roledata._base._id:ToStr() then
		...
		player:Log("OnCommand_MafiaKickout, error=MAFIA_KICKOUT_NO_SELF")
		return
	end
	...
	if mafia_data == nil then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MafiaKickout, error=NO_MAFIA")
		return
	end
	...
	if role._roledata._mafia._position ~= G_MAFIA_POSITION["BANGZHU"] and role._roledata._mafia._position ~= G_MAFIA_POSITION["JUNSHI"] then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MafiaKickout, error=MAFIA_SET_LEVEL_ERR")
		return
	end
	...
	if find_member == nil then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MafiaKickout, error=MAFIA_KICKOUT_NO_ROLE")
		return
	end
	...
	if role._roledata._mafia._position >= find_member._position then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MafiaKickout, error=MAFIA_KICKOUT_NO_POWER")
		return
	end
