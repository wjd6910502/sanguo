function OnCommand_MallBuyItem(player, role, arg, others)
	...
	if arg.item_num <= 0 then
		...
		player:Log("OnCommand_MallBuyItem, error=MALLITEM_ID_ERR")
		return
	end
	...
	if mallitem.limit_times_id ~= 0 then
		...
		if LIMIT_TestUseLimit(role, mallitem.limit_times_id, arg.item_num) == false then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_MallBuyItem, error=MALLITEM_MAX_ERR")
			return
		end
	end
	...
	if currency.currency_type == 1 then
		...
		if mallitem.limit_times_id == 0 then
			if currency.price[1] <= 0 then
				...
				player:SendToClient(SerializeCommand(resp))
				player:Log("OnCommand_MallBuyItem, error=MALLITEM_TEM_MONEY_ERR")
				return
			end
			...
			if buy_count < arg.item_num then
				...
				if max_price == 0 then
					...
					player:SendToClient(SerializeCommand(resp))
					player:Log("OnCommand_MallBuyItem, error=MALLITEM_TEM_ERR")
					return
				end
		...
		if needmoney > role._roledata._status._money then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_MallBuyItem, error=MALLITEM_MONEY_LESS")
			return
		end
		...
		local flag, all_items = BACKPACK_AddItem(role, mallitem.itemid, mallitem.nums*arg.item_num)
		resp.item = {}
	...
	elseif currency.currency_type == 2 then
		...
		local needmoney = 0
		if mallitem.limit_times_id == 0 then
			if mallitem.price[1] <= 0 then
				resp.retcode = G_ERRCODE["MALLITEM_TEM_MONEY_ERR"]
				player:SendToClient(SerializeCommand(resp))
				player:Log("OnCommand_MallBuyItem, error=MALLITEM_TEM_MONEY_ERR")
				return
			end
			needmoney = arg.item_num*mallitem.price[1]
		else
			local count = LIMIT_GetUseLimit(role, mallitem.limit_times_id)
			local flag = 0
			local buy_count = 0
		...
		if needmoney > role._roledata._status._yuanbao then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_MallBuyItem, error=MALLITEM_YUANBAO_LESS")
			return
		end
	...
	elseif currency.currency_type == 4 then
		...
		if needmoney > repnum then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_MallBuyItem, error=MALLITEM_REP_LESS")
			return
		end
