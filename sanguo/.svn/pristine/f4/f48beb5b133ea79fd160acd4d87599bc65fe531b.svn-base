function OnCommand_TowerSweep(player, role, arg, others)
	...
	if arg.layer ~= role._roledata._tower_data._cur_layer then
		...
		player:Log("OnCommand_TowerSweep, error=TOWER_ERROR_ARG")
		return
	end
	...
	if towerstage == nil then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_TowerSweep, error=TOWER_ERROR_DATA")
		return
	end
	...
	if towerstage.stage_category ~= 1 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_TowerSweep, error=TOWER_ERROR_TYPE")
		return
	end
	...
	if towerstage1 == nil or towerstage1.can_skipped_level < arg.layer then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_TowerSweep, error=TOWER_ERROR_SWEEP")
		return
	end
