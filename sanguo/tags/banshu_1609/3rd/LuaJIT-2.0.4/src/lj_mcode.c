...
#define SZ	32768
struct _mcode_node_t
{
	struct _mcode_node_t *_next;
	struct _mcode_node_t *_prev;
	void *_addr;
};
static struct _mcode_node_t *_free_mem = NULL;
static struct _mcode_node_t *_used_mem = NULL;
...
static void mcode_free(jit_State *J, void *p, size_t sz)
{
  ...
  munmap(p, sz);
#endif
	//fprintf(stderr, "mcode_free, p=%p, sz=%d\n", p, (int)sz);

	munmap(p, sz);

	if(!_used_mem || sz!=SZ) return;

	char found = 0;
	struct _mcode_node_t *nd = _used_mem;
	while(nd)
	{
		if(nd->_addr == p)
		{
			//fprintf(stderr, "mcode_free found, p=%p, sz=%d\n", p, (int)sz);
			found = 1;
			//remove
			if(nd->_prev)
			{
	...
	void *q = mmap(p, sz, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
	if(q == p)
	{
		//fprintf(stderr, "mcode_free reget, p=%p, sz=%d\n", p, (int)sz);
		//ok, prepend to _free_mem
		nd->_next = 0;
		nd->_prev = 0;
		nd->_addr = q;
		if(_free_mem)
...
#ifdef LJ_TARGET_POSIX
int luajit_mcode_pre_alloc()
{
#if LJ_TARGET_MIPS
...
#endif
	const uintptr_t range = (1u << (LJ_TARGET_JUMPRANGE-1)) - (1u << 21); //14M

	void *begin = NULL;
	if(target > (range>>1))
	{
		begin = (void*)(target - (range>>1));
	}

	void *end = (void*)(target + (range>>1));

	int count = 0;
	while(begin<end && begin+SZ<end)
	{
		if(begin > 0)
		{
			void *p = mmap(begin, SZ, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
			if(p == begin)
			{
				//ok, prepend to _free_mem
				struct _mcode_node_t *nd = malloc(sizeof(struct _mcode_node_t));
				nd->_next = 0;
				nd->_prev = 0;
				nd->_addr = p;
				if(_free_mem)
...
{
#ifdef LJ_TARGET_POSIX
	//fprintf(stderr, "mcode_alloc, sz=%d\n", (int)sz);
	...
	struct _mcode_node_t *nd = _free_mem;
	_free_mem = _free_mem->_next;
	if(_free_mem)
	{
		_free_mem->_prev = 0;
	}
	...
	//fprintf(stderr, "mcode_alloc ok, addr=%p\n", nd->_addr);
	return nd->_addr;
#else
