...
public:
	...
	void OnReceiveData(int64_t now_ms, bool to_svr, const string& data)
	{
		...
		if(to_svr)
		{
			if(_to_svr_lost_packet_percent>0 && (rand()%100)<_to_svr_lost_packet_percent) return; //lost
		...
		{
			if(_to_cli_lost_packet_percent>0 && (rand()%100)<_to_cli_lost_packet_percent) return; //lost
