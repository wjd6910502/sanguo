...
private:
	...
	static int mppc_compress(const unsigned char *ibuf, register unsigned char *obuf, int isize, int osize)
	{
		...
		//如果obuf是通过栈传递进来的，而不是在寄存器中，那么下面这条语句在６４位平台下赋值的时候可能被截断。
		//现有编译环境生成的代码没有问题。如果还有其它问题再修改。
