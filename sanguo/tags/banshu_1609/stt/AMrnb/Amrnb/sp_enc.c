      ...
      Float32 gain_pit, Float32 en[], Float32 *ltpg )
{
   ...
   if ( en[3] > 0 && en[0] != 0 ) {
      /* gain = ResEn / LTPResEn */
      pred_gain = en[0] / en[3];
      *ltpg = ( Float32 )( log10((double)pred_gain ) / log10((double)2 ) );
   }
   else {
      *ltpg = 0;
   }
...
static void vad_pitch_detection( vadState *st, Word32 T_op[] )
{
   ...
   for ( i = 0; i < 2; i++ ) {
      if ( abs( double(st->oldlag - T_op[i]) ) < LTHRESH ) {
         lagcount += 1;
      }
