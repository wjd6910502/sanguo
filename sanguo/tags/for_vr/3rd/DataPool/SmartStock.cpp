...
{
  static wch szok[] = {'o','k','\0'};
  static wch sztrue[] = {'t','r','u','e','\0'};
  static wch szyes[] = {'y','e','s','\0'};
  static wch sz1[] = {'1','\0'};
  return clstd::strcmpiT(pStr, szok) == 0 ||
    clstd::strcmpiT(pStr, sztrue) == 0 ||
    clstd::strcmpiT(pStr, szyes) == 0 ||
    clstd::strcmpiT(pStr, sz1) == 0;
}
  ...
  b32 _SSP_IMPL::Close()
{
  ...
  for(HandleArray::iterator it = m_aHandles.begin();
    it != m_aHandles.end(); ++it) {
      SECTION_DESC* pDesc = (SECTION_DESC*)(*it);
      SAFE_DELETE(pDesc);
  }
  m_aHandles.clear();
  ...
  b32 _SSP_IMPL::SECTION_DESC::SetKey( T_LPCSTR szKey, T_LPCSTR szValue )
{
  ...
  if(GetKey(szKey, param)) {
    ...
    strBuffer.Append('=');
    strBuffer.Append(SmartStreamUtility::MakeQuotation(strBuffer, szValue));
    strBuffer.Append(';');
    strBuffer.Append('\r');
    strBuffer.Append('\n');
