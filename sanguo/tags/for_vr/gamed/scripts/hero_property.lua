...
function Property_UpdateHeroInfo(hero_info)
	...
	return Property_Zhanli(weapon_property, equip_property, relation_property, hero_property, weapon_skill_property, legion_info)
end

--战力的计算,变量分别代表的是武器，装备，羁绊，武将本身，特效带来的影响
function Property_Zhanli(weapon_property, equip_property, relation_property, hero_property, weapon_skill_property, legion_info)
	--weapon_property.atk
	--weapon_property.def
	...
	for index = 1, table.getn(legion_info) do
		...
		for learn_index = 1, table.getn(legion_info[index].learned) do
			...
			if legion_learn_info ~= nil then
				legion_add_atk = legion_add_atk + legion_learn_info.add_atk ;
				legion_add_maxhp = legion_add_maxhp + legion_learn_info.add_maxhp ;
				legion_add_armor = legion_add_armor + legion_learn_info.add_armor ;
			end
		end
	end
