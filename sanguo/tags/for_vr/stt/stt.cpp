...
extern "C" 
{
    ...
    std::string iat_GetText( std::string sessionid, unsigned char* p_pcm,unsigned int pcm_size,const char* session_begin_params)
	{
		...
		char			hints[HINTS_SIZE]			=	{NULL}; //hintsÎª½áÊø±¾´Î»Ø»°µÄÔ­ÒòÃèÊö£¬ÓÉÓÃ»§×Ô¶¨Òå
		unsigned int	total_len					=	0; 
		int				aud_stat					=	MSP_AUDIO_SAMPLE_CONTINUE ;		//ÒôÆµ×´Ì¬
		int				ep_stat						=	MSP_EP_LOOKING_FOR_SPEECH;		//¶Ëµã¼ì²â
		int				rec_stat					=	MSP_REC_STATUS_SUCCESS ;		//Ê¶±ğ×´Ì¬
