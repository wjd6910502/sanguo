...
public:
	Link(int64_t now_ms, const sockaddr_in& cli_addr, int to_svr_fd, int to_svr_delay_ms, int to_cli_delay_ms, int close_period_ms)
	    : _to_svr_delay_ms(to_svr_delay_ms), _to_cli_delay_ms(to_cli_delay_ms), _close_period_ms(close_period_ms), _to_svr_fd(to_svr_fd), _last_receive_data_time_ms(now_ms)
	{
...
public:
	...
	Link* CreateLink(int64_t now_ms, const sockaddr_in& cli_addr, int to_svr_fd, int to_svr_delay_ms, int to_cli_delay_ms, int close_period_ms)
	{
		Link *link = new Link(now_ms, cli_addr, to_svr_fd, to_svr_delay_ms, to_cli_delay_ms, close_period_ms);
				...
				if(ret > 0)
				{
					...
					if(fd == to_cli_fd)
					{
						...
						if(!link)
						{
							...
							link = LinkManager::GetInstance().CreateLink(now_ms, src_addr, to_svr_fd, to_svr_delay_ms, to_cli_delay_ms, close_period_ms);
						}
