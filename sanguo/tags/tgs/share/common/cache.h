...
namespace GNET
{
	...
	public:
		...
		void RDLock()					{	_locker.RDLock();	}
		void WRLock()					{	_locker.WRLock();	}
		void UNLock()					{	_locker.UNLock();	}

		void RDLock(const _Key & __key)	{	_locker.RDLock();	}
		void WRLock(const _Key & __key)	{	_locker.WRLock();	}
	...
	public:
		...
		~BackSave()
		{
			if(_savetask) _savetask->Destroy();
			_savetask = NULL;
	...
	public:
		...
		~LRUDiscard()
		{
			if(_discardtask) _discardtask->Destroy();
			_discardtask = NULL;
		}
