...
static int script_ResetCmdLockMisc(lua_State *l)
{
	g_cmd_lock_mist_map.clear();
	return 0;
}

static int script_SetCmdLockMisc(lua_State *l)
{
	int n = lua_gettop(l);
	if(n != 1) return 0;
...
static int script_ResetMsgLockMisc(lua_State *l)
{
	g_msg_lock_mist_map.clear();
	return 0;
}

static int script_SetMsgLockMisc(lua_State *l)
{
	int n = lua_gettop(l);
	if(n != 1) return 0;
...
lua_State* InitLuaEnv()
{
	...
	if(l)
	{
		...
		lua_register(l, "API_SetCmdLockMisc", script_SetCmdLockMisc);
		lua_register(l, "API_ReSetCmdLockMisc", script_ResetCmdLockMisc);
		//msg
		...
		lua_register(l, "API_ResetMsgLockMisc", script_ResetMsgLockMisc);
		lua_register(l, "API_SetMsgLockMisc", script_SetMsgLockMisc);
