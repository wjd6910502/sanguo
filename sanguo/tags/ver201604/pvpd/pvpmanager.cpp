...
int PVP::CheckFighter(time_t now, PVPFighter& fighter)
{
	//断线检查
...
void PVP::OnTimer1s(time_t now)
{
	...
	{
		//PVPHeartbeat
		int ret = CheckFighter(now, _fighter1);
		if(ret)
		{
		...
		ret = CheckFighter(now, _fighter2);
		if(ret)
		{
...
//timeval tv;
//gettimeofday(&tv, 0);
//GLog::log(LOG_INFO, "%d-%d: OP, fighter=%ld, client_tick=%d, op_encoded=%s", (int)tv.tv_sec, (int)tv.tv_usec/1000, fighter, client_tick, op_encoded);
//}
		...
		while(true)
		{
			...
			tmp.first_operation = Octets((void*)it1->second.c_str(), it1->second.size());
			tmp.second_operation = Octets((void*)it2->second.c_str(), it2->second.size());
...
void PVP::SendOperation(int win_flag)
{
	PvpOperation prot;
...
int PVPManager::Create(int id, int mode, int64_t fighter1, int64_t fighter2, int fight_start_time, int typ, Octets fighter1_pvpinfo, Octets fighter2_pvpinfo, int fighter1_zoneid, int fighter2_zoneid)
{
	...
	_map[id] = new PVP(id, mode, fighter1, fighter2, fight_start_time, typ, fighter1_pvpinfo, fighter2_pvpinfo, fighter1_zoneid, fighter2_zoneid);
...
void PVPManager::SendOperation(int id, int win_flag)
{
	...
	it->second->SendOperation(win_flag);
}
