...
#define my_max(N,M) ((N)>(M)? N : M)
#define my_min(N,M) ((N)<(M)? N : M)
...
public:
  ...
  bool trace_D_path() {
    ...
    assert(D < 0x7fffffff); //TODO make this an error case?
  ...
  bool is_overlapped(Vector &forward, Position &bisect) {
    ...
    int32_t kb = my_max(size_delta - kEnd, kBegin);
    int32_t ke = my_min(size_delta - kBegin, kEnd);
