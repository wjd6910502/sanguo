...
function BACKPACK_AddItem(role, tid, count)
	return BACKPACK_ActualAddItem(role, tid, count, 1)
end

function BACKPACK_ActualAddItem(role, tid, count, layer)
	if layer >= 3 then
		...
		for z = 1, count do
			for i = 1, item.type_data2 do
				...
				for j = 1, table.getn(Item) do
					BACKPACK_ActualAddItem(role, Item[j].id, Item[j].count, layer+1)
				end
			end
		end
		return true
...
function BACKPACK_DelEquipment(role, equipment_id)
	API_Log("11111111111111111111111111111111111111111")
