...
function ROLE_ResetBattleInfo(role, battle_id)
	...
	for npc in DataPool_Array(battlefielddata_info.init_npcs) do
		...
		tmp_npc._alive = 1
		tmp_npc._event_flag = 0
		tmp_npc._army_buff = 0
		--找到信息放进去
...
function ROLE_GetBattleEventInfo(role, battleid, eventid)
	...
	if battleevent_info.defeatarmyid ~= 0 then
		local position_it = battle._position_info:SeekToBegin()
		...
		while position_info ~= nil do
			...
			local npc = npc_it:GetValue()
			while npc ~= nil do
				if npc._armyid == battleevent_info.defeatarmyid and npc._alive == 1 then
					local tmp_del = {}
					tmp_del.id = position_info._id
					tmp_del.pos = position_info._position
					tmp_del.npc_id = npc._id
					tmp_del.army_id = battleevent_info.defeatarmyid
	...
	if battleevent_info.joinarmyid ~= 0 then
		local position = battle._position_info:Find(battleevent_info.joinarmypos)
		if position ~= nil then
			...
			if alive_army == 0 then
				battle._npc_id = battle._npc_id + 1
				local tmp_npc = CACHE.BattleFieldNPCData()
				tmp_npc._id = battle._npc_id
				tmp_npc._camp = battleevent_info.joinarmycamp
				tmp_npc._armyid = battleevent_info.joinarmyid
				tmp_npc._alive = 1
				position._npc_info:Insert(tmp_npc._id, tmp_npc)
				if battleevent_info.joinarmycamp == 1 or battleevent_info.joinarmycamp == 3 then
					position._flag = 1
				elseif battleevent_info.joinarmycamp == 2 then
					position._flag = 2
				end
	...
	for pos_buff in DataPool_Array(battleevent_info.change_slot_buff) do
		if pos_buff.slotpos ~= 0 then
			...
			if position ~= nil then
				position._pos_buff = pos_buff.slotbuff
			...
			while position_info ~= nil do
				...
				while npc ~= nil do
					if npc._armyid == army_buff.armyid and npc._alive == 1 then
						npc._army_buff = army_buff.armybuff

						local tmp_army_buff = {}
						tmp_army_buff.id = position_info._id
						tmp_army_buff.pos = position_info._position
						tmp_army_buff.army = npc._armyid
						tmp_army_buff.buff_id = army_buff.armybuff
