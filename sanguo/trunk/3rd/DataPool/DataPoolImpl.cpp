...
#define ERROR_CODE_NOT_FOUND_VAR              6006
  ...
  GXHRESULT DataPool::CreateDataPool(DataPool** ppDataPool, const TYPE_DECLARATION* pTypeDecl, const VARIABLE_DECLARATION* pVarDecl)
  {
//#ifdef DATAPOOLCOMPILER_PROJECT
    GXHRESULT hval = GX_OK;

    DataPoolImpl* pDataPoolImpl = new DataPoolImpl();
...
//#endif // #ifdef DATAPOOLCOMPILER_PROJECT
  ...
  GXHRESULT DataPool::CreateFromFile(DataPool** ppDataPool, AFile& file, GXDWORD dwFlag)
  {
    GXHRESULT hval = GX_OK;

    DataPool* pDataPool = new DataPoolImpl();
  ...
  GXHRESULT DataPool::CreateFromFileW(DataPool** ppDataPool, GXLPCWSTR szFilename, GXDWORD dwFlag)
  {
      return GX_E_OPEN_FAILED;
#if 0
    AFile file;
    clStringA strFilename = szFilename;
    if( ! file.Open(strFilename, AFILE_OPENEXIST)) {
      return GX_E_OPEN_FAILED;
    }
    return CreateFromFile(ppDataPool, file, dwFlag);
#endif
  }

  GXHRESULT DataPool::CreateFromFileImageW(DataPool** ppDataPool, GXLPCWSTR szFilename, GXDWORD dwFlag)
  {
      return GX_E_OPEN_FAILED;
#if 0
	  AFileImage file;
  ...
  GXHRESULT DataPool::CreateFromFileA(DataPool** ppDataPool, GXLPCSTR szFilename, GXDWORD dwFlag)
  {
    AFile file;
  ...
  GXHRESULT DataPool::CreateFromFileImageA (DataPool** ppDataPool, GXLPCSTR szFilename, GXDWORD dwFlag)
  {
      return GX_E_OPEN_FAILED;
#if 0
	  AFileImage file;
...
#endif
    IMPORT import;
    GXWCHAR szMsgName[] = {'d','p','c','m','s','g','.','t','x','t','\0'};
    import.ErrorMsg.LoadErrorMessageW(szMsgName);
...
#define _DPEM_CLS    DataPoolErrorMsg<_TChar>
  ...
  void _DPEM_CLS::WriteErrorW(GXBOOL bError, GXSIZE_T nOffset, GXUINT nCode, ...)
  {
    ...
    if(it == m_ErrorMsg.end()) {
      static GXWCHAR szMissingMessage[] = {'M','i','s','s','i','n','g',' ','C','o','m','p','i','l','e','r',' ','e','r','r','o','r',' ','m','e','s','s','a','g','e','.','\r','\n','\0'};
      str.AppendFormat(szMissingMessage);
    }
    else {
