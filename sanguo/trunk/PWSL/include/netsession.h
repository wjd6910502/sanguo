...
protected:
	...
	bool	_weak_associate_io;		//和IO弱关联，在此状态下，关闭session不会导致关闭IO 但会调用IO的另外一个接口
...
protected:
	...
	NetSession(const NetSession &rhs) : _assoc_io(rhs._assoc_io), _sending(false), _closing(false), _close_discard(rhs._close_discard),_weak_associate_io(rhs._weak_associate_io),_status(0),
...
public:
	...
	struct Config
	{
		...
		Config(): ibuffermax(DEFAULTIOBUF), obuffermax(DEFAULTIOBUF), bufferthreshold(0), decrypt_type(0), encrypt_type(0), close_discard(false),latency_mode(0),
			socket_type(GST_TCP), port(0), so_sndbuf(0), so_rcvbuf(0), nodelay(1), broadcast(0), listen_backlog(0)
		{
			verify_peer = 0;
			ssldebug = 0;
		}
		...
		bool Load(const char *identification, Conf *pconf = NULL)
		{
			...
			if(!strcasecmp(type.c_str(), "tcp")) socket_type = GST_TCP; else
			if(!strcasecmp(type.c_str(), "udp")) socket_type = GST_UDP; else
			if(!strcasecmp(type.c_str(), "unix")) socket_type = GST_UNIX; else 
			if(!strcasecmp(type.c_str(), "unixio")) socket_type = GST_UNIX_FD; else return false;
	...
	virtual void Init()
	{
		...
		_weak_associate_io = false;	//此值默认为false，需主动调用设置为true
	...
	void SetWeakAssoc(bool value) { _weak_associate_io = value;}

	NetSession() : _assoc_io(NULL), _sending(false), _closing(false), _close_discard(false),_weak_associate_io(false),_status(0),
...
private:
	...
	virtual void OnOpen() { }
	void DoOpen(const SockAddr& local, const SockAddr& peer) { _local_addr = local, _peer_addr = peer; OnOpen(local, peer);}
	void DoOpen() { OnOpen();}
