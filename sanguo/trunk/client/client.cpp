...
public:
	...
	virtual void OnRecvGameProtocol(const Octets& lua_prot)
	{
		...
		if(g_L)
		{
			...
			LuaWrapper lw(g_L);
			if(!lw.gExec("DeserializeAndProcessCommand", LuaParameter((void*)0, (void*)0, std::string((char*)lua_prot.begin(), lua_prot.size()))))
			{
				fprintf(stderr, "OnRecvGameProtocol, gExec, DeserializeAndProcessCommand, error, %.*s\n", (int)lua_prot.size(), (char*)lua_prot.begin());
			}
		}
	}
