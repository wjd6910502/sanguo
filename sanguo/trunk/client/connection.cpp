...
void FastUDPSession::OnAck(int index_ack)
{
	...
	if(it2 != _time_map.end())
	{
		_time_map.erase(_time_map.begin(), ++it2);
	}
}
...
void FastUDPSession::SetReceived(int index)
{
	...
	assert(index==_received_index+1); //FIXME: 上线前删掉吧
...
void FastUDPSession::TriggerSend(bool force, int retrans_time_ms)
{
	if(!_opened) return;
	if(_data_map.empty() && !_need_send_ack) return;

	//GameClient *gc = Connection::GetInstance().GetGameClient();
	GameClient *gc = _conn->GetGameClient();
	...
	if(!_data_map.empty() && !_need_send_ack)
	{
		...
		if(it != _time_map.end())
		{
			...
			it = _time_map.begin();
			int64_t t = it->second;
			if(now_ms-t<retrans_time_ms) return;
		}
	}
	...
	for(auto it=_data_map.begin(); it!=_data_map.end(); ++it)
	{
		...
		if(_time_map.find(it->first)==_time_map.end()) _time_map[it->first]=now_ms;
	...
	if(!_p2p)
	{
		...
		prot.server_send_time = _conn->GetPrevServerSendTime4PVP() + (now_micro-_conn->GetPrevServerSendTime4PVPLocalTime())/1000;
		...
		prot.signature = UDPSignature(tmp, _conn->GetKey1());

		//UDPTransClient::GetInstance()->SendTo(gc->GetPVPDIP(), gc->GetPVPDPort(), prot);
		_conn->GetUDPTransClient()->SendTo(gc->GetPVPDIP(), gc->GetPVPDPort(), prot);
	}
	else
	{
		std::string ip;
                      ...
                      const char *status_ip, unsigned short status_port)
{
	...
	if(!_gate_client)
	{
		_gate_client = new GateClient();
		_gate_client->SetConnection(this);
	}
	GateClient *manager = _gate_client;
	manager->CloseCur();
...
void Connection::Close()
{
	...
	if(_gate_client) _gate_client->CloseCur();
	//TransClient::GetInstance()->CloseCur();
	if(_trans_client) _trans_client->CloseCur();
	//UDPTransClient::GetInstance()->CloseCur();
	//StatusClient::GetInstance()->CloseCur();
	if(_status_client) _status_client->CloseCur();
}
                                  ...
                                  const std::vector<int>& extra_pvps)
{
	...
	if(_status==CONN_STATUS_ESTABLISHED && _can_send_game_protocol)
	{
		//TransClient::GetInstance()->SendProtocol(prot);
		_trans_client->SendProtocol(prot);
	}
	else
	{
		_callback->OnSendGameProtocolTimeout();
		_game_protocol_timeouting = true;
	}

	AddHistory(prot, now_micro/1000000+3); //timeout: 10
}
...
void Connection::SendUDPGameProtocol(const Octets& data)
{
	...
	_udp_trans_client->SendTo(_trans_ip.c_str(), _trans_port, prot);
}
...
void Connection::OnChallenge(Protocol::Manager *manager, int sid, const Octets& server_rand1)
{
	...
	prot.res_ver = Octets("0.0.1", strlen("0.0.1"));
                              ...
                              const Octets& trans_token)
{
	...
	if(retcode == ERR_CODE_WRONG_VERSION)
	{
		Close();

		_callback->OnClientOutOfDate();
		return;
	}
	else if(retcode != 0)
	{
	...
	if(!_trans_client)
	{
		_trans_client = new TransClient();
		_trans_client->SetConnection(this);
	}
	TransClient *manager2 = _trans_client;
	manager2->CloseCur();
	Protocol::Client(manager2);

	if(!_udp_is_init)
	{
		_udp_trans_client = new UDPTransClient();
		_udp_trans_client->SetConnection(this);

		_udp_is_init = true;
		//UDPTransClient *manager3 = UDPTransClient::GetInstance();
		UDPTransClient *manager3 = _udp_trans_client;
...
void Connection::OnTransAuthResult(Protocol::Manager *manager, int sid, int retcode, int server_received_count, bool do_reset)
{
	...
	if(retcode != 0)
	{
		...
		GateClient *manager2 = _gate_client;
		if(manager2)
		{
			manager2->CloseCur();
			Protocol::Client(manager2);
...
void Connection::OnContinue(Protocol::Manager *manager, int sid, bool reset)
{
	...
	if(reset)
	{
		...
		_callback->OnTimeoutClear();
		_game_protocol_timeouting = false;
...
bool Connection::Update()
{
	...
	int rt = _pvp_latency.Get();
	if(rt<0 || rt>200) rt=200;
	rt = rt/33*33;

	_udp_session.TriggerSend(false,rt);
	if(P2P_IsEstablished()) _p2p_udp_session.TriggerSend(false, 0);
	...
	if(!_game_protocol_timeout.empty())
	{
		...
		{
			_callback->OnSendGameProtocolTimeout();
			_game_protocol_timeout.begin()->second = now+3;
			_game_protocol_timeouting = true;
		}
	}
...
//if((now%7) == 1)
{
		...
		}
		{
			...
			_udp_trans_client->SendTo(GetUDPTransIP(), GetUDPTransPort(), prot);
		}
		{
			ServerLog prot;
			...
			if(prot.logs.size()>0) _trans_client->SendProtocol(prot);
		}
}
		return true;
	}
	...
	if(--_timeout <= 0)
	{
		...
		case CONN_STATUS_WAIT_CHALLENGE:
		{
			...
			if(_gate_client)
			{
				_gate_client->DiscardPendingSession();
				_gate_client->CloseCur();
			}

			//StatusClient *manager2 = StatusClient::GetInstance();
			if(!_status_client)
			{
				_status_client = new StatusClient();
				_status_client->SetConnection(this);
			}
			StatusClient *manager2 = _status_client;
			if(manager2)
			{
				manager2->CloseCur();
				Protocol::Client(manager2);
		...
		case CONN_STATUS_WAIT_TRANS_CHALLENGE:
		{
			...
			if(_trans_client)
			{
				_trans_client->DiscardPendingSession();
				_trans_client->CloseCur();
			}
			...
			else
			{
				...
				TransClient *manager = _trans_client;
				if(manager)
				{
					manager->CloseCur();
					Protocol::Client(manager);
		...
		case CONN_STATUS_WAIT_SERVER_STATUS:
		{
			...
			if(_status_client)
			{
				_status_client->DiscardPendingSession();
				_status_client->CloseCur();
			}
...
void P2PHelper::TryMakeHole(int magic, const char *peer_ip, unsigned short peer_port)
{
	...
	_conn->GetUDPTransClient()->SendTo(_make_hole_peer_ip.c_str(), _make_hole_peer_port, prot);
}
...
void P2PHelper::SendTest1ToGamed()
{
	...
	prot.change_port = 0;
	//UDPTransClient::GetInstance()->SendTo(Connection::GetInstance().GetUDPTransIP(), Connection::GetInstance().GetUDPTransPort(), prot);
	_conn->GetUDPTransClient()->SendTo(_conn->GetUDPTransIP(), _conn->GetUDPTransPort(), prot);
}
...
void P2PHelper::SendTest1ToStund()
{
	...
	_conn->GetUDPTransClient()->SendTo(_conn->GetStundIP(), _conn->GetStundPort(), prot);
}

void P2PHelper::GetNetType_ChangeStatus(P2P_GET_NET_TYPE_STATUS status)
{
	_get_net_type_status = status;
	_get_net_type_magic++; //区分不同的状态，避免延迟到达的response互相干扰
...
void Connection::UpdateServerReceivedCount(int server_received_count)
{
	...
	if(_game_protocol_timeouting && _game_protocol_timeout.empty())
	{
		_callback->OnTimeoutClear();
		_game_protocol_timeouting = false;
	}
