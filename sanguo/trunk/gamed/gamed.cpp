...
//	CACHE::SGT_Mafia_Info::GetInstance().OnTimer(_tick, now);
	...
	CACHE::SGT_HotPvpVideo::GetInstance().OnTimer(_tick, now);
	CACHE::SGT_ServerReward::GetInstance().OnTimer(_tick, now);
...
int main(int argc, char *argv[])
{
	...
	{
		GMAdapterServer *manager = GMAdapterServer::GetInstance();
		manager->SetAccumulate(atoi(conf->find(manager->Identification(), "accumulate").c_str()));
		Protocol::Server(manager);
	}

	{
		GACCodeClient *manager = GACCodeClient::GetInstance();
		manager->SetAccumulate(atoi(conf->find(manager->Identification(), "accumulate").c_str()));
		Protocol::Client(manager);
	}
	
	{
		LaohuInformServer *manager = LaohuInformServer::GetInstance();
		manager->SetAccumulate(atoi(conf->find(manager->Identification(), "accumulate").c_str()));
		Protocol::Server(manager);
	}
