...
static int script_BILog(lua_State *l)
{
	int n = lua_gettop(l);
	if(n != 1) return 0;
	...
	GLog::log(LOG_NOTICE, "===LUA LOG===: %s", log);
	return 0;
}

static int script_GetTime(lua_State *l)
{
	lua_pushinteger(l, (int)Now());

	return 1;
}

static int script_GetTimeStr(lua_State *l)
{
	time_t now = Now();
	char buf[100];
	lua_pushstring(l, ctime_r(&now, buf));
...
{
	lua_pushnumber(l, NowUS()/1000);

	return 1;
}

int64_t g_uptime = 0;
static int script_GetUptime(lua_State *l) //服务器启动后多久了，毫秒计量
{
	lua_pushnumber(l, (NowUS()-g_uptime)/1000);
...
static int script_SetTimeOffset(lua_State *l)
{
	int n = lua_gettop(l);
	if(n != 2) return 0;
	...
	if(hour || minute) g_db_save_flag=0;

	return 0;
}

static int script_HaveTimeOffset(lua_State *l)
{
	lua_pushboolean(l, (g_db_save_flag==0));
	return 1;
}

static int script_SetOpenServerOffset(lua_State *l)
{
	int n = lua_gettop(l);
	if(n != 1) return 0;
...
static int script_MakeTodayTime(lua_State *l)
{
	...
	time_t now = Now();
	struct tm tmp;
	//struct tm *tmp = localtime_r(&now, &tmp);
	localtime_r(&now, &tmp);
	tmp.tm_hour = hour;
	tmp.tm_min = minute;
	tmp.tm_sec = second;
...
{
	assert(g_server_state==SERVER_STATE_LOADING_END);

	Thread::HouseKeeper::AddTimerTask(DelayDBSaveDataTask::GetInstance(), SERVER_CONST_SAVE_INTERVAL);
...
static int script_GetActiveRoleCount(lua_State *l)
{
	lua_pushnumber(l, PlayerManager::GetInstance().GetActiveRoleCount()); //TODO
	return 1;
}

extern int g_zoneid;
static int script_GetZoneId(lua_State *l)
{
	lua_pushnumber(l, g_zoneid);
	return 1;
}
...
lua_State* InitLuaEnv()
{
	...
	if(l)
	{
		...
		lua_register(l, "API_BILog", script_BILog);
		lua_register(l, "API_GetTime", script_GetTime);
		lua_register(l, "API_GetTimeStr", script_GetTimeStr);
		lua_register(l, "API_GetTime2", script_GetTime2);
		lua_register(l, "API_GetUptime", script_GetUptime);
		lua_register(l, "API_IsNULL", script_IsNULL);
		lua_register(l, "API_ReloadLua", script_ReloadLua);
		lua_register(l, "API_SetTimeOffset", script_SetTimeOffset);
		lua_register(l, "API_HaveTimeOffset", script_HaveTimeOffset);
		lua_register(l, "API_SetOpenServerOffset", script_SetOpenServerOffset);
		...
		lua_register(l, "API_GetActiveRoleCount", script_GetActiveRoleCount);
		lua_register(l, "API_GetZoneId", script_GetZoneId);
		//customerservice
		...
		if(luaL_dofile(l, "./scripts/postinit.lua"))
		{
			fprintf(stderr, "InitLuaEnv, error, luaL_dofile(postinit.lua)\n");
			lua_close(l);
			delete mem;
			l = 0;
			return l;
