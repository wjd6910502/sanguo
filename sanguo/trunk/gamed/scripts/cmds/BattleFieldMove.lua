function OnCommand_BattleFieldMove(player, role, arg, others)
	...
	if battle == nil then
		...
		player:Log("OnCommand_BattleFieldMove, error=BATTLE_ID_NOT_EXIST")
		return
	end
	...
	if battle._state == 0 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_BattleFieldMove, error=BATTLE_NOT_BEGIN")
		return
	elseif battle._state == 2 then
		resp.retcode = G_ERRCODE["BATTLE_HAVE_END"]
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_BattleFieldMove, error=BATTLE_HAVE_END")
		return
	end
	...
	if battle._cur_position ~= arg.src_position then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_BattleFieldMove, error=BATTLE_MOVE_SRC_ERR")
		return
	end
	...
	if battle._attacked_flag ~= 0 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_BattleFieldMove, error=BATTLE_STATE_NO_MOVE")
		return
	end
	...
	if received_flag ~= 1 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_BattleFieldMove, error=BATTLE_MOVE_DST_ERR")
		return
	end
