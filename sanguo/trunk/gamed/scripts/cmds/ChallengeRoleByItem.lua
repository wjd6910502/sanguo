function OnCommand_ChallengeRoleByItem(player, role, arg, others)
	...
	if dest_role == nil then
		...
		player:Log("OnCommand_ChallengeRoleByItem, error=JJC_CHALLENGE_ROLE_NOT_EXIST")
		return
	end
	...
	if BACKPACK_HaveItem(role, 17250, 1) == false then
		...
		role:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_ChallengeRoleByItem, error=JJC_ITEM_NOT_EXIST")
		return
	end
	...
	resp.role_info.photo = dest_role._roledata._base._photo
	resp.role_info.photo_frame = dest_role._roledata._base._photo_frame
	resp.role_info.rank = 0
	resp.role_info.badge_info = {}
	...
	while badge_info ~= nil do
		...
		resp.role_info.badge_info[#resp.role_info.badge_info+1] = tmp_badge_info
	...
	role._roledata._pve_arena_info._cur_attack_player._badge_map = dest_role._roledata._base._badge_map
