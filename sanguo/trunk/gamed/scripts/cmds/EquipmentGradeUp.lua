function OnCommand_EquipmentGradeUp(player, role, arg, others)
	...
	if typ_flag == 0 then
		...
		player:Log("OnCommand_EquipmentGradeUp, error=EQUIPMENT_TYP_ERR")
		return
	end
	...
	if find_equipment == nil then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentGradeUp, error=EQUIPMENT_NOT_EXIST")
		return
	end
	...
	if find_equipment._equipment_pro._hero_id ~= arg.hero then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentGradeUp, error=EQUIPMENT_BIND_OTHER_HERO")
		return
	end
	...
	if hero_info == nil then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentGradeUp, error=EQUIPMENT_HERO_NOT_EXIST")
		return
	end
	...
	if equipment_info.equip_grade_id == 0 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentGradeUp, error=EQUIPMENT_NOT_GRADE")
		return
	end
	...
	if find_equipment._equipment_pro._order >= equipment_grade_info.max_grade then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_EquipmentGradeUp, error=EQUIPMENT_GRADE_MAX")
		return
	end
