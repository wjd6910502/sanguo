function OnCommand_FriendReply(player, role, arg, others)
	...
	if s_role==nil then
		--如果玩家不存在的话，那么就判断一下是否在申请列表里面，在的话就删除掉。免得一个一直不上线的玩家占了一个名额
		...
		player:Log("OnCommand_FriendReply, error=FRIEND_REPLY_ROLE_NOT_EXIST")
		return
	end
		...
		if r._brief._id:ToStr()==s_role._roledata._base._id:ToStr() then
			if arg.accept == true then
				...
				if friends:Size() >= quanju.friends_max then
					...
					player:SendToClient(SerializeCommand(resp))
					player:Log("OnCommand_FriendReply, error=FRIEND_REQUEST_SELF_MAX")
					return
				end
				...
				if src_friends:Size() >= quanju.friends_max then
					...
					player:SendToClient(SerializeCommand(resp))
					player:Log("OnCommand_FriendReply, error=FRIEND_REQUEST_OTHER_MAX")
					return
				end
				...
				if role._roledata._friend._blacklist:Find(s_role._roledata._base._id) ~= nil then
					...
					player:SendToClient(SerializeCommand(resp))
					player:Log("OnCommand_FriendReply, error=FRIEND_REQUEST_SELF_BLACK_LIST")
					return
				end
				...
				if s_role._roledata._friend._blacklist:Find(role._roledata._base._id) ~= nil then
					...
					player:SendToClient(SerializeCommand(resp))
					player:Log("OnCommand_FriendReply, error=FRIEND_REQUEST_OTHER_BLACK_LIST")
					return
				end

				--互加好友
				local nf = CACHE.Friend()
				...
				nf._brief._photo_frame = role._roledata._base._photo_frame
				nf._brief._badge_map = role._roledata._base._badge_map
				nf._sex = role._roledata._base._sex
				...
				nf._brief._photo_frame = s_role._roledata._base._photo_frame
				nf._brief._badge_map = s_role._roledata._base._badge_map
				nf._sex = s_role._roledata._base._sex
				...
				cmd.friend.photo_frame = role._roledata._base._photo_frame
				cmd.friend.sex = role._roledata._base._sex
				
				cmd.friend.badge_info = {}
				...
				while badge_info ~= nil do
					...
					cmd.friend.badge_info[#cmd.friend.badge_info+1] = tmp_badge_info
				...
				cmd.friend.photo_frame = s_role._roledata._base._photo_frame
				cmd.friend.sex = s_role._roledata._base._sex
				
				cmd.friend.badge_info = {}
				local badge_info_it = s_role._roledata._base._badge_map:SeekToBegin()
				local badge_info = badge_info_it:GetValue()
				while badge_info ~= nil do
					local tmp_badge_info = {}
					tmp_badge_info.id = badge_info._id
