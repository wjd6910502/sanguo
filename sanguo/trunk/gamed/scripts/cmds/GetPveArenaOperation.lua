function OnCommand_GetPveArenaOperation(player, role, arg, others)
	...
	if history_info == nil then
		...
		player:Log("OnCommand_GetPveArenaOperation, error=JJC_VIDEO_NOT_EXIST")
		return
	end
	...
	if history_info._operation._checksum ~= G_CHECKSUM_S["PveArenaOperation"] then
		player:Log("OnCommand_GetPveArenaOperation, Err in deserialize struct: strus has changed!")
		return
	end
	local is_idx,ds_hero_info = DeserializeStruct(history_info._operation._str, 1, "PveArenaOperation")
	resp.operation = {}
