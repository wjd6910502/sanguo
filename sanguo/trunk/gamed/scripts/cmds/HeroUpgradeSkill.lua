...
function UpGradeSkill(role, hero_id, skill_info)
	...
	if hero == nil then
		player:Log("OnCommand_HeroUpgradeSkill, error=NO_HERO")
	...
	for i=1,#skill_info do
		...
		if skillpackage.functionname == "" then
			player:Log("OnCommand_HeroUpgradeSkill, error=HERO_SKILL_NOT_LEVELUP")
		...
		elseif common_skill ~= nil then
			...
			player:Log("OnCommand_HeroUpgradeSkill, error=HERO_NO_SKILLID")
		...
		elseif skill_curr_level == skill_level then
			...
			player:Log("OnCommand_HeroUpgradeSkill, error=HERO_SKILLLV_WRONG_PARAM")
	...
	if hero._cur_skill_point <= 0 or  hero._cur_skill_point	< skillpoints then 	
		player:Log("OnCommand_HeroUpgradeSkill, error=HERO_SKILL_POINT_LESS")
		return G_ERRCODE["HERO_SKILL_POINT_LESS"]
	end
	
	-- 钱够不够
	if role._roledata._status._money < needmoney then
		player:Log("OnCommand_HeroUpgradeSkill, error=HERO_SKILLLV_MONEY_LESS")
