function OnCommand_JieYiInviteRole(player, role, arg, others)
	...
	if dest_role == nil then
		...
		player:Log("OnCommand_JieYiInviteRole, error=JIEYI_INVITEROLE_NOT_EXIST")
		return
	end
	...
	if dest_role._roledata._status._online == 0 then
		...
		role:SendToClient(SerializeCommand(cmd))
		player:Log("OnCommand_JieYiInviteRole, error=JIEYI_INVITEROLE_NOT_ONLINE")
		return
	end
	...
	if dest_role._roledata._jieyi_info._jieyi_id:ToStr() ~= "0" then
		...
		role:SendToClient(SerializeCommand(cmd))
		player:Log("OnCommand_JieYiInviteRole, error=JIEYI_INVITEROLE_HAS_JIEYI")
		return
	end
	...
	if dest_role._roledata._status._level < 1 then
		...
		role:SendToClient(SerializeCommand(cmd))
		player:Log("OnCommand_JieYiInviteRole, error=JIEYI_INVITEROLE_LEVEL_NOT_ENOUGH")
		return	
	end
	...
	if role._roledata._jieyi_info._cur_operate_typ == 1 then	   							 	
		...
		if info ==nil then
			...
			role:SendToClient(SerializeCommand(cmd))
			player:Log("OnCommand_JieYiInviteRole, error=JIEYI_INFO_NOT_FOUND")
			return
		end
	elseif role._roledata._jieyi_info._cur_operate_typ == 2 then
		...
		role:SendToClient(SerializeCommand(cmd))
		player:Log("OnCommand_JieYiInviteRole, error=JIEYI_REQUEST_TYPE_WRONG")
		return
	end
	...
	if info._brother_info:Size() >= 2 then	
		...
		role:SendToClient(SerializeCommand(cmd))
		player:Log("OnCommand_JieYiInviteRole, error=JIEYI_INVITEROLE_NUMBER_SURPASS")
		return
	end
	...
	elseif resp.typ == 2 then
		...
		role:SendToClient(SerializeCommand(cmd))
		player:Log("OnCommand_JieYiInviteRole, error=JIEYI_INVITEROLE_NOT_CREATED")
		return
	end
