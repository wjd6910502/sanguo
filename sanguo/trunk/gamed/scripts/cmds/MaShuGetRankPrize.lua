function OnCommand_MaShuGetRankPrize(player, role, arg, others)
	...
	if role._roledata._mashu_info._yestaday_rank == 0 then
		...
		player:Log("OnCommand_MaShuGetRankPrize, error=MASHU_GET_PRIZE_NOT_RANK")
		return
	end
	...
	if role._roledata._mashu_info._get_prize_flag == 1 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MaShuGetRankPrize, error=MASHU_GET_PRIZE_HAVE_GET")
		return
	end
	...
	if reward_id == 0 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_MaShuGetRankPrize, error=MASHU_GET_PRIZE_RANK_LOW")
		return
	end
