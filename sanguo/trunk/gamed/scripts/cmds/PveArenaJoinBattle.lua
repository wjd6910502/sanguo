function OnCommand_PveArenaJoinBattle(player, role, arg, others)
	...
	if role._roledata._pve_arena_info._defence_hero_info:Size() == 0 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_PveArenaJoinBattle, error=JJC_HERO_ZERO")
		return
	end
	...
	if LIMIT_GetUseLimit(role, quanju.arena_free_times) == 0 then
		role._roledata._pve_arena_info._pve_refreshtime = now + quanju.arena_recovery_times_cd
		--更新
		local respp = NewCommand("PveArenaUpdateRefreshTime")
		respp.pve_refreshtime = role._roledata._pve_arena_info._pve_refreshtime
		role:SendToClient(SerializeCommand(respp))	
	end
	...
	if fight_info.role_id ~= "0" then
		...
		role._roledata._pve_arena_info._cur_attack_player._zhanli = fight_info.zhanli
