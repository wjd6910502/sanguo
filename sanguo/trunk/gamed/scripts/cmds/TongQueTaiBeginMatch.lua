function OnCommand_TongQueTaiBeginMatch(player, role, arg, others)
	...
	if cur_pvp_ver <= 0 then
		...
		player:Log("OnCommand_TongQueTaiBeginMatch, error=EXE_OUT_OF_DATE")
		return
	end
	...
	if role._roledata._tongquetai_data._reward_flag == 1 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_TongQueTaiBeginMatch, error=TONGQUETAI_REWARD_NOT_GET")
		return
	end
	...
	if role._roledata._tongquetai_data._hero_info_list:Size() < 3 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_TongQueTaiBeginMatch, error=TONGQUETAI_JOIN_HERO_LESS")
		return
	end
	...
	if role._roledata._tongquetai_data._cur_state ~= 0 then
		...
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_TongQueTaiBeginMatch, error=TONGQUETAI_JOIN_IN_ACTIVITY")
		return
	end
	...
	if LIMIT_TestUseLimit(role, quanju.party_for_flag_limit_id, 1) == true then
		...
		if cost_num == 0 or cost_typ == 0 then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_TongQueTaiBeginMatch, error=TONGQUETAI_JOIN_SYS_COST_ERR")
			return
		end
		...
		if arg.double_flag == 1 then
			if role._roledata._status._yuanbao < quanju.party_for_flag_3_times_reward_cost then
				...
				player:SendToClient(SerializeCommand(resp))
				player:Log("OnCommand_TongQueTaiBeginMatch, error=TONGQUETAI_JOIN_COST_MONEY_LESS")
				return
			end
		end
		...
		elseif cost_typ == 2 then
			...
			player:SendToClient(SerializeCommand(resp))
			player:Log("OnCommand_TongQueTaiBeginMatch, error=TONGQUETAI_JOIN_SYS_COST_TYP_ERR")
			return
		end
	else
		resp.retcode = G_ERRCODE["TONGQUETAI_JOIN_NO_COUNT"]
		player:SendToClient(SerializeCommand(resp))
		player:Log("OnCommand_TongQueTaiBeginMatch, error=TONGQUETAI_JOIN_NO_COUNT")
		return
	end
