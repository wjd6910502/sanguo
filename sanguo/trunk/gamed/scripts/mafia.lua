function MAFIA_MakeMafia(mafia)
	...
	ret.declaration_time = mafia._declaration_broadcast_time
	--_member_map
	ret.members = {}
...
function MAFIA_MafiaUpdateInterfaceInfo(mafia)
	...
	msg.info.declaration_time = mafia._declaration_broadcast_time
...
function MAFIA_MafiaSendMail(mafia, mail_id, arg1)
	--扔一个消息去处理
...
function MAFIA_MafiaChat(mafia, mafia_msg)
	--扔一个消息去处理
	local msg = NewMessage("PublicChatnew")
	msg.src = mafia_msg.src
	msg.text_content = mafia_msg.text_content
	msg.channel = mafia_msg.channel
	msg.speech_content = mafia_msg.speech_content
	msg.time = mafia_msg.time
	msg.chat_typ = mafia_msg.chat_typ

	MAFIA_BroadcastMsg(mafia, SerializeMessage(msg), 0)
end

function MAFIA_MafiaYueZhan(mafia, msg)
	--扔一个消息去处理
	local mafia_msg = NewMessage("YueZhanInfo")
	mafia_msg.id = msg.id
	...
	MAFIA_BroadcastMsg(mafia, SerializeMessage(mafia_msg), 0)
end
