function OnMessage_RoleUpdateServerEvent(player, role, arg, others)
	...
	local server_event = others.misc._miscdata._server_event._event_info
	...
	while event ~= nil do
		if event._event_time > role._roledata._status._update_server_event then
			if event._event_type == G_EVENT_TYPE["MAIL_PVP_DAILY"] then
				...
				local mail_id = 8001
				mail_id = mail_id + role._roledata._pvp_info._pvp_grade
			...
			elseif event._event_type == G_EVENT_TYPE["PVP_SEASON_FINISH"] then
				role._roledata._pvp_info._pvp_server_season_end_time = others.misc._miscdata._pvp_season_end_time
				role._roledata._pvp_info._pvp_season_end_time = others.misc._miscdata._pvp_season_end_time
				...
				for ach in DataPool_Array(achs) do
					if ach.achtype == G_ACH_TYPE["PVP_SERVER_FINISH"] then
						...
						if role._roledata._status._level < ach.aquire_level then
							flag = 0
						end

						if role._roledata._pvp_info._pvp_grade >= ach.start_range and 
						role._roledata._pvp_info._pvp_grade <= ach.end_range and flag == 1 then
