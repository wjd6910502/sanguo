function OnMessage_SendMail(player, role, arg, others)
	...
	mail._subject = arg.title
	mail._context = arg.content
	...
	--查看是否需要给帮会发邮件
	if arg.mafia_mail_id ~= 0 then
		if role._roledata._mafia._id:ToStr() ~= "0" then
			local msg = NewMessage("SendMailToMafia")
			msg.mail_id = arg.mafia_mail_id
			msg.arg1 = role._roledata._base._name
			API_SendMsg(role._roledata._mafia._id:ToStr(), SerializeMessage(msg), 0)
		end
	end
	return
end
