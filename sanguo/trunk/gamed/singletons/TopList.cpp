...
SGT_TopList::SGT_TopList(): Singleton("SGT_TopList")
{
...
void SGT_TopList::backup(const char *name, int64_t transaction_id)
{
...
void SGT_TopList::restore(int64_t transaction_id)
{
	if(transaction_id == _transaction_id)
	{
...
void SGT_TopList::Load(Octets &key, Octets &value)
{
	...
	else
	{
		...
		else if(str_key == "toplist_tower_big")
		{
			_data._top_data.Insert(TOPLIST_TYPE_TOWER_BIG, top_value);	
		}
		else if(str_key == "toplist_dati")
		{
			_data._top_data.Insert(TOPLIST_TYPE_DATI, top_value);	
		}
...
void SGT_TopList::Save()
{
	...
	for(it_manager = _data._top_data.SeekToBegin(); (toplist = it_manager.GetValue()); it_manager.Next())
	{
		...
		else if(toplist->_top_list_type == TOPLIST_TYPE_TOWER_BIG)
		{
			std::string key_str = "toplist_tower_big";
			
			value << db_version;
			toplist->marshal(value);
			g_save_data_map[Octets(key_str.c_str(), key_str.size())] = value;	
		}
		else if(toplist->_top_list_type == TOPLIST_TYPE_DATI)
		{
			std::string key_str = "toplist_dati";
