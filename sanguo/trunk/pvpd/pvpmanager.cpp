...
int PVP::CheckFighter(time_t now, PVPFighter& fighter, int big_lag_count)
{
	...
	if(fighter._jiasu_count>PVP_CONST_JIASU_OP_MAX) return PVP_ERROR_JIASU;
		...
		if(_mode == 1)
		{
			...
			if(_status == PVP_STATUS_PAUSE_AND_WAIT_RESP1)
			{
				...
				else if(now > _status_timeout)
				{
					...
					prot.reason = (_fighter1._have_sin ? PVP_ERROR_NO_PAUSE_RE : 100+PVP_ERROR_NO_PAUSE_RE);
					if(_typ == 0)
					{
						PVPGameClient::GetInstance()->SendProtocol(prot);
					}
			...
			else if(_status == PVP_STATUS_PAUSE_AND_WAIT_RESP2)
			{
				if(now > _status_timeout-10) //FIXME: 临时方案
				{
					...
					_fighter1._jiasu_count = 0;
					_fighter2._jiasu_count = 0;
					return;
				}
				else if(now > _status_timeout)
				{
			...
			if(ret)
			{
				if(_fighter1._pause_count < PVP_CONST_PAUSE_COUNT_MAX)
				{
					...
					_first_tick = _next_client_tick;
					_fighter1._waiting_pause_re = true;
					_fighter2._waiting_pause_re = true;
					_fighter1._have_sin = true;
					_fighter2._have_sin = false;
			...
			if(ret)
			{
				if(_fighter2._pause_count < PVP_CONST_PAUSE_COUNT_MAX)
				{
					...
					_fighter1._waiting_pause_re = true;
					_fighter2._waiting_pause_re = true;
					_fighter1._have_sin = false;
					_fighter2._have_sin = true;
...
{
	return _fight_start_time*(int64_t)1000 + (int)((client_tick-_first_tick)*1000.0f/PVP_CONST_CLIENT_FPS);
}
...
void PVP::OnPVPOperation(int64_t fighter, int client_tick, const char *op_encoded, int crc_tick, const char *crc_encoded)
{
	if(_robot_flag == 1)
	{
		if(_mode == 1)
		{
			...
			//timeval tv;
			//gettimeofday(&tv, 0);
			//GLog::log(LOG_INFO, "%d-%d: PVP Operation, fighter=%ld, client_tick=%d, op_encoded=%s", (int)tv.tv_sec, (int)tv.tv_usec/1000, fighter, client_tick, op_encoded);
			//}
	...
	else
	{
		if(_mode == 1)
		{
			...
			//timeval tv;
			//gettimeofday(&tv, 0);
			//GLog::log(LOG_INFO, "%d-%d: PVP Operation, fighter=%ld, client_tick=%d, op_encoded=%s, crc_tick=%d, crc_encoded=%s", (int)tv.tv_sec, (int)tv.tv_usec/1000, fighter, client_tick, op_encoded, crc_tick, crc_encoded);
			//}
			...
			int64_t should = GetShouldArriveTime(client_tick);
			if(now_ms+20<should)
			{
				GLog::log(LOG_INFO, "PVPD::PVP::OnPVPOperation, jiasu, pvp=%d, fighter=%ld, client_tick=%d, now_ms=%ld, should=%ld, _fight_start_time=%d, _first_tick=%d", _id, fighter, client_tick, now_ms, should, _fight_start_time, _first_tick);
				ft->_jiasu_count++; //客户端疑似加速了
			}
			...
			if(ft->_crcs.find(crc_tick) == ft->_crcs.end())
			{
				//这个只是用来辅助差错，简单处理
				ft->_crcs[crc_tick] = crc_encoded;
				auto ito = oft->_crcs.find(crc_tick);
				if(ito != oft->_crcs.end())
				{
					if(ito->second != crc_encoded)
					{
						//出现不一致了!
						GLog::log(LOG_INFO, "PVPD::PVP::OnPVPOperation, inconsistent, pvp=%d, crc_tick=%d, END", _id, crc_tick);

						char buf[1024];
						snprintf(buf, sizeof(buf)-1, "10319:");

						Player *player1 = PlayerManager::GetInstance().Find(_fighter1._id);
						if(player1) player1->FastSess_Send(buf, false);
						Player *player2 = PlayerManager::GetInstance().Find(_fighter2._id);
						if(player2) player2->FastSess_Send(buf, false);
					}
				}
			}
...
void PVP::SendOperation(int win_flag, int robot_flag, int robot_seed, int robot_type)
{
	PvpOperation prot;
	prot.id = _id;
...
void PVPManager::SendOperation(int id, int win_flag, int robot_flag, int robot_seed, int robot_type)
{
	...
	it->second->SendOperation(win_flag, robot_flag, robot_seed, robot_type);
}
