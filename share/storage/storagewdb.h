...
# define TO_LITTLE_ENDIAN_64(x) (x)
#else
# if __BYTE_ORDER == __BIG_ENDIAN
#  define TO_LITTLE_ENDIAN_64(x)     bswap_64(x)
# endif
#endif
...
//////////////////////////////////////////////////////////////////////////////////
//
//	db文件锁: 防止同一db被多个db程序误写
//
//	  db程序(gamedbd/dbview等)运行时会在dbhome目录下生成一个lock文件
//	  lock文件保存了当前db程序进程id
//	  如果此时有其他db程序要使用这些db文件，则会检查到lock文件而报错退出
static int _TestLock(const char *path) //>0表示可使用, <0表示Test出错或db被其他进程锁定, ==0表示还需要再测试一次
{
	char buf[100];
	int _fd = open(path, O_CREAT|O_EXCL|O_WRONLY, 0644);
	if (_fd >= 0) //没有锁, 加锁
	{
	...
	if (errno == EEXIST) //已经有锁
	{
		...
		{
			//其他进程加的锁, 并且加锁进程已经不在了
			unlink(path); //FIXME: 有争用, 不好解决, 但预计概率很小
			return 0; //retry
		}
	}
	return -1;
...
static bool TestLock(const char *path)
{
	int r = _TestLock(path);
	if (r < 0)
		return false;
	else if (r == 0)
	{
...
public:
	...
	public:
		...
		void insert( const int64_t id, const GNET::Octets &val, Transaction &txn, int flags = 0 )
		{
			GNET::Octets key;
			int64_t leid = TO_LITTLE_ENDIAN_64(id);
			key.insert(key.end(), &leid, sizeof(leid));
			insert(key, val, txn, flags);
		}
		...
		GNET::Octets find( const int64_t id, Transaction &txn )
		{
			GNET::Octets key;
			int64_t leid = TO_LITTLE_ENDIAN_64(id);
			key.insert(key.end(), &leid, sizeof(leid));
		...
		bool find( const int64_t id, GNET::Octets &val, Transaction &txn )
		{
			GNET::Octets key;
			int64_t leid = TO_LITTLE_ENDIAN_64(id);
			key.insert(key.end(), &leid, sizeof(leid));
			return find(key, val, txn);
		}
		...
		void del( const int64_t id, Transaction &txn, int flags = 0 )
		{
			GNET::Octets key;
			int64_t leid = TO_LITTLE_ENDIAN_64(id);
			key.insert(key.end(), &leid, sizeof(leid));
		...
		void* first(GNET::Octets &key, GNET::Octets &val)
		{
			try
			{
				char key_buf[32], val_buf[4096];
				size_t key_len = sizeof(key_buf), val_len = sizeof(val_buf);
				void *pkey = key_buf, *pval = val_buf;
				void *cur = db->first(pkey, key_len, pval, val_len);
				if (cur) {
					key = GNET::Octets(pkey, key_len);
					uncompressor->Update(GNET::Octets(pval, val_len)).swap(val);
					if (pkey != key_buf) {
						free(pkey);
					}
					if (pval != val_buf) {
		...
		void* next(void *prev, GNET::Octets &key, GNET::Octets &val)
		{
			try
			{
				char key_buf[32], val_buf[4096];
				size_t key_len = sizeof(key_buf), val_len = sizeof(val_buf);
				void *pkey = key_buf, *pval = val_buf;
				void *cur = db->next(prev, pkey, key_len, pval, val_len);
				if (cur) {
					key = GNET::Octets(pkey, key_len);
					uncompressor->Update(GNET::Octets(pval, val_len)).swap(val);
					if (pkey != key_buf) {
	...
	class MultiTableIQuery
	{
	public:
		virtual ~MultiTableIQuery() { }
		virtual bool Update( const GNET::Octets& key, const std::vector<GNET::Octets>& vals ) = 0;
	};

	class MultiTableCursor
	{
		class Exception {};
		class EmptyTable: public Exception {};
		class LastRecord: public Exception {};
		...
		public:
			Cursor(Storage *t): table(t)
			{
				begin = table->first(key, val);
				if (!begin) throw EmptyTable();
			}
			const GNET::Octets& GetKey() const { return key; }
			const GNET::Octets& GetVal() const { return val; }
			void Next()
			{
				begin = table->next(begin, key, val);
				if (!begin) throw LastRecord();
			}
		};
		...
		static int Compare(const GNET::Octets& a, const GNET::Octets& b)
		{
			if ( int comp = GNET::__db_helper::memcmp_fast( a.begin(), b.begin(), std::min(a.size(), b.size()) ) )
				return comp;
			return a.size() - b.size();
		}

		static GNET::Octets GetTheSame(Cursor& c1, Cursor& c2)
		{
			while (true) {
				int r = Compare(c1.GetKey(), c2.GetKey());
				if (r < 0) c1.Next();
				else if (r > 0) c2.Next();
				else return c1.GetKey();
			}
		}

		GNET::Octets GetTheSame()
		{
			if (cursors.size() == 1) return cursors[0].GetKey();
			if (cursors.size() == 2) return GetTheSame(cursors[0], cursors[1]);

			GNET::Octets last;
			for (size_t i = 1; i < cursors.size(); i++) {
			        GNET::Octets c = GetTheSame(cursors[0], cursors[i]);
			        if (i == 1) {
			                last = c;
			        } else if (Compare(c, last) > 0) {
			                // redo
			                i = 0;
			                continue;
			        } else if (i == cursors.size() - 1) {
			                return c;
			        }
			}
			throw 999;      // never
	...
	public:
		void add_storage(Storage* st)
		{
			if (std::find(storages.begin(), storages.end(), st) != storages.end()) return;
			storages.push_back(st);
		}

		void walk(MultiTableIQuery& query, Transaction& txn)
		{
			...
			{
				txn.lock();
				for (std::vector<Storage*>::iterator it = storages.begin(), ie = storages.end(); it != ie; ++it) {
					cursors.push_back(Cursor(*it));
				}
				while (true) {
					GNET::Octets key = GetTheSame();
					std::vector<GNET::Octets> vals;
					for (std::vector<Cursor>::iterator it = cursors.begin(), ie = cursors.end(); it != ie; ++it) {
						vals.push_back(it->GetVal());
					}
					if (!query.Update(key, vals)) break;
					cursors[0].Next();
				}
			}
			catch ( GNET::__db_helper::PageFile::Exception e )
...
public:
	...
	static bool Open()
	{
		...
		if (!TestLock((homedir+"/.dblock").c_str())) //加个检查，防止同一db被多个程序读写
		{
			fprintf(stderr, "StorageEnv::Open() failed: lock conflict.\n");
			return false;
		}
	...
	{
		bool value = true;
		__db_helper::Page::page_align(&value);
	}
	...
	static bool checkpoint(bool reset=false)
	{
		if(reset)
		{
			std::for_each( storage_vec.begin(), storage_vec.end(), std::mem_fun(&Storage::reset) );
			return true;
		}
		try
		{
